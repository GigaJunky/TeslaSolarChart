...
let date = process.argv[2] ? process.argv[2] : now.toISOString().substr(0, 10)
...
TeslaAPI.requestData({ kind: 'products' }, (err, res) => {
    ...
    TeslaAPI.requestData({ kind: 'vehicles' }, (err, res) =>
        console.log(err, res)
    )
    TeslaAPI.requestData({ kind: 'honk_horn' }, (err, res) =>
        console.log(err, res)
    )
})