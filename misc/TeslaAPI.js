...
function createOptions(query) {
    ...
    let cmd = '', method = 'GET'
    switch (query.kind) {
        case 'honk_horn': cmd = `/api/1/vehicles/${vehicle_id}/command/honk_horn`;  method = 'POST'; break
        case 'wake_up'  : cmd = `/api/1/vehicles/${vehicle_id}/wake_up`;  method = 'POST'; break
        case 'vehicles' : cmd = `/api/1/vehicles/${vehicle_id}/vehicle_data`; break

        case 'energy':
        case 'power':
            cmd = `/api/1/energy_sites/${energy_site_id}/history?kind=${query.kind}&date=${qdate}&period=${query.period}&time_zone=${time_zone}`
            break
        default: cmd = `/api/1/products`; break
    }
    console.log('cmd:', cmd)
    return {
        method: method, host: 'owner-api.teslamotors.com', port: 443, path: cmd,
        headers: {
